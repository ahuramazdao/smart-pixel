...
export function Input({ label, placeholder, type = 'text', value, onChange, required = false, error, dark = false, style: extra = {} }) {
    ...
    <label style={{ display: 'block', minWidth: 0, ...extra }}>
      ...
      <input
        ...
        style={{
          ...
          border: `1px solid ${error ? 'var(--sp-err, #C81E00)' : focus ? 'var(--sp-red, #F0391C)' : dark ? 'var(--sp-line-d, rgba(255,255,255,.14))' : 'var(--sp-line, #E2DFD7)'}`,
          boxShadow: focus ? '0 0 0 3px rgba(240,57,28,.14)' : 'none',
          transition: 'border-color 140ms, box-shadow 140ms',
        }}
      />
      {error && <span style={{ ...mono, color: 'var(--sp-err, #C81E00)', display: 'block', marginTop: 6 }}>{error}</span>}
    </label>
  );
}
